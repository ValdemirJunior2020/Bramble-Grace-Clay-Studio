...
def render_clay_motion(project:Project,scene:Scene,audio:Path,timings:list,target:Path,preview:bool=False,progress:Callable|None=None,mouth_cues:list|None=None)->Path:
    ...
    try:
        for i in range(frames):
            ...
            if scene.blocks:
                sec=i/fps; active=None; cursor=0.0
...
def render_scene(project:Project,scene:Scene,language:str,preview:bool=False,progress:Callable|None=None)->Path:
    ...
    render_clay_motion(project,scene,audio,[],base,preview,progress);style=scene.subtitle_override or project.settings.subtitle;final=outdir/f'{scene.number:03}-{scene.id}.mp4'
    if style.enabled and style.burn_in:_burn(base,srt,final,style);base.unlink(missing_ok=True)
    else:base.replace(final)

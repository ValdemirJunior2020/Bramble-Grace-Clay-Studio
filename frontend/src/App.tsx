...
function Editor({p,chars,setP,save,importStory,addImages,replaceImage,patchScene,move,switchLang,render,setMsg}:any){
...
</div>}{s.blocks.map((b:any)=><div className="speaker" key={b.id}><select value={b.speaker||''} onChange={e=>{const blocks=s.blocks.map((x:any)=>x.id===b.id?{...x,speaker:e.target.value,needs_review:false}:x);patchScene(s,{blocks})}}><option value="">Speaker needs review</option><option value="Narrator">Narrator</option>{chars.filter((c:Character)=>c.id!=='narrator').map((c:Character)=><option key={c.id} value={c.name}>{c.name}</option>)}</select><textarea value={b.text} readOnly/></div>)}</div><div><button className="btn" onClick={()=>move(s,-1)}>↑</button><button className="btn" onClick={()=>move(s,1)}>↓</button><label className="btn">Replace Image<input hidden type="file" accept="image/*" onChange={e=>{const f=e.target.files?.[0];if(f)replaceImage(s,f);e.currentTarget.value=''}}/></label><button className="btn primary" onClick={()=>render('preview',s)}>Preview</button>{s.preview_url&&<video controls width="210" src={media(s.preview_url)}/>}</div></div>)}</>}

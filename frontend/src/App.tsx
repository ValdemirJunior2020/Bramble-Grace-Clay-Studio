...
export default function App(){
 ...
 const importStory=async(file?:File,text?:string)=>{if(!project)return;const fd=new FormData();if(file)fd.append('file',file);if(text)fd.append('text',text);setProject(await api(`/api/projects/${project.id}/import-story`,{method:'POST',body:fd}));};
 ...
 {page==='editor'&&project&&<Editor p={project} chars={chars} setP={setProject} save={save} importStory={importStory} addImages={addImages} patchScene={patchScene} move={move} switchLang={switchLang} render={render}/>} 

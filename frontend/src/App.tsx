...
export default function App(){
 ...
 const animateScene=async(s:Scene)=>{if(!project)return;try{setMsg(`Preparing Scene ${s.number} Animator...`);const lang=project.settings.language==='pt-br'?'pt-br':'en';const ctx=await api<any>(`/api/projects/${project.id}/scenes/${s.id}/motionity-context?language=${lang}`,{method:'POST'});const q=new URLSearchParams({image:ctx.image_url||'',audio:ctx.audio_url||'',width:String(ctx.width||1920),height:String(ctx.height||1080),duration:String(ctx.duration||5),scene:String(ctx.scene_number||s.number)});window.open(`${ctx.motionity_url}?${q.toString()}`,'_blank','noopener,noreferrer');setMsg(`Scene ${s.number} opened in Motionity.`)}catch(e:any){setMsg(e?.message||'Could not open Motionity. Run INSTALL-MOTIONITY.bat and restart Clay Studio.')}};
 ...
 {page==='editor'&&project&&<Editor p={project} chars={chars} setP={setProject} save={save} importStory={importStory} addImages={addImages} replaceImage={replaceImage} animateScene={animateScene} patchScene={patchScene} move={move} switchLang={switchLang} render={render} setMsg={setMsg}/>} 
 {page==='characters'&&<Characters chars={chars} refresh={load}/>} 
 {page==='system'&&<System data={system}/>} 
 {msg&&<div className="pill warn">{msg}</div>}{queue.length>0&&<div className="queue"><div className="queue-head"><b>Generation Queue</b><span>{queue.filter((j:any)=>j.state==='running'||j.state==='queued').length} active</span></div>{queue.slice(0,4).map((j:any)=>{const pct=Math.round((j.progress||0)*100);const label=j.kind==='render_story'?'Full Movie':j.kind==='preview_scene'?'Scene Preview':j.kind==='render_scene'?'Scene Render':'Generation';const done=j.state==='complete';const failed=j.state==='failed';const active=j.state==='running'||j.state==='queued';return <div key={j.id} className={'queue-job '+(done?'done':failed?'failed':active?'active':'')}><div className="queue-row"><div><div className="queue-title">{label}</div><div className="queue-stage">{done?'Complete — file ready':failed?j.stage:(j.stage||'Preparing')}</div></div><div className={'queue-badge '+(done?'done':failed?'failed':'working')}>{done?'Complete':failed?'Failed':j.state==='queued'?'Queued':pct+'%'}</div></div><div className="progress"><div className={active?'progress-fill moving':'progress-fill'} style={{width:(Math.max(done?100:pct,2))+'%'}}/></div><div className="queue-foot"><span>{done?'Finished':failed?'Stopped':active?'Rendering in progress':'Waiting'}</span><span>{done?'100%':pct+'%'}</span></div>{done&&j.result_path&&<div className="queue-result">Saved: {j.result_path}</div>}{j.error&&<div className="queue-error"><b>Details:</b> {j.error}<div style={{marginTop:4,opacity:.85}}>Full traceback: logs/render.log</div></div>}</div>})}</div>}
...
function Editor({p,chars,setP,save,importStory,addImages,replaceImage,animateScene,patchScene,move,switchLang,render,setMsg}:any){
